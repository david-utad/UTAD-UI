...
void AUTAD_UI_FPSCharacter::BeginPlay()
{
  ...
  if (SkillTreeWidget)
  {
    m_pSkillTreeInstance = CreateWidget<USkillTreeWidget>(GetWorld(), SkillTreeWidget);
    m_pSkillTreeInstance->AddToViewport();
  }
...
void AUTAD_UI_FPSCharacter::SetupPlayerInputComponent(class UInputComponent* PlayerInputComponent)
{
    ...
    //Skill Tree
    m_pSkillTreeAction->bTriggerWhenPaused = true;
    EnhancedInputComponent->BindAction(m_pSkillTreeAction, ETriggerEvent::Started, this, &AUTAD_UI_FPSCharacter::SkillTreeScreen);
	}
}
...
void AUTAD_UI_FPSCharacter::SkillTreeScreen()
{
  if (m_pSkillTreeInstance != nullptr)
  {
    if (m_pSkillTreeInstance->IsVisible())
    {
      if (bHasRifle)
      {
      ...
      m_pSkillTreeInstance->Hide();
    }
    else
    {
      PlayerHUDInstance->Hide();
      m_pSkillTreeInstance->Show();
    }
  }
}
